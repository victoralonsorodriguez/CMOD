...
for filter_system_index, filter_system in enumerate(filter_name_group_sorted):
    ...
    plt.savefig(f'{cwd}/{galaxy}_plot_galfit_{filter_name_title}_{psf_size}_{version}_mag_ser_radeff.pdf', format='pdf', dpi=1000, bbox_inches='tight')

    # Closing the plot to avoid overlapse
    plt.close()    


print('All galfit plots for magnitude, Sérsic index and effective radius are done')
print('#------------------------------------------------------------------------#\n')