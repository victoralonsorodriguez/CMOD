...
for filter_system_index, filter_system in enumerate(filter_name_group_sorted):
    ...
    # Title of the plot
    fig.suptitle(f'{galaxy} ratios with {filter_name_title} filters system with a {psf_size} - {version}',
    ...
    plt.savefig(f'{cwd}/{galaxy}_plot_ratios_{filter_name_title}_{psf_size}_{version}_ser_radeff.pdf', format='pdf', dpi=1000, bbox_inches='tight')
    # Closing the plot to avoid overlapse
    plt.close()    

print('All galfit plots for Sérsic index and efective radius ratios are done')
print('#-------------------------------------------------------------------#\n')
