...
def version_file(galaxy_name, version, analysis_version, psf_size,archivo):

    datetime_current = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    date, time = datetime_current.split(' ')
    
    nueva_linea = f'VERSION={version} DATE={date} TIME={time} ANALYSIS_MODE={analysis_version} PSF={psf_size}\n'
