...
def plot_gen(x_data=[],
    ...
    for data_pos in range(len(x_data)):
        if y_axis_diff == False or data_pos+1 != y_axis_diff_val_pos:
            ...
            if style_plot[data_pos] == 'sct':
                ax.scatter(np.array(x_data[data_pos])/x_con_factor,
                        np.array(y_data[data_pos])/y_con_factor,
                        label=f'{label_list[data_pos]}',
                        linewidth=0.15,
                        edgecolor='black',
                        color=line_color[data_pos],
                        s=mark_size[data_pos],
                        marker=mark_style[data_pos],
                ...
                ax.plot(np.array(x_data[data_pos])/x_con_factor,
                        np.array(y_data[data_pos])/y_con_factor,
                        linewidth=0.5,
                        color=line_color[data_pos],
                        linestyle=line_style[data_pos],
                        zorder=zorder[data_pos]-1)
                ...
                ax.plot(np.array(x_data[data_pos])/x_con_factor,
                        ...
                        color=line_color[data_pos],
                        linestyle=line_style[data_pos],
                        linewidth=width_style[data_pos],
                        zorder=zorder[data_pos],
                        alpha=line_alpha[data_pos])
                
        else:
            axyrig = ax.twinx() 
            if style_plot[data_pos] == 'sct-line':
                axyrig.scatter(np.array(x_data[data_pos])/x_con_factor,
                        np.array(y_data[data_pos])/y_con_factor,
                        label=f'{label_list[data_pos]}',
                        linewidth=0.15,
                        edgecolor='black',
                ...
                axyrig.plot(np.array(x_data[data_pos])/x_con_factor,
                        np.array(y_data[data_pos])/y_con_factor,
                        linewidth=0.5,
                        color=line_color[data_pos],
                        linestyle=line_style[data_pos],
    ...
    if guide_lines[0]==True:
        ...
        if guide_lines[1][1] != None and len(guide_lines[1][1])>0:
            for line_pos in guide_lines[1][0]:
                ax.axhline(line_pos, color="black",linestyle="dashdot", linewidth=1, 
                       zorder=0, alpha=0.9)
    ...
    ### Eje Y izquierdo
    if y_axis_diff == True:
        ax.set_ylabel(f'{y_axis_label[0]}',fontsize=axis_font_size,labelpad=10)
    else:
    ...
    # Datos para plotear
    y_axis_l_data = [y for y in y_data]
    ...
    if y_axis_diff == False:
        ...
        # Si tenemos eje secundario diferente 
        # nos quedamos con sus valore
        y_axis_diff_r_data = [y for y in y_data]
        y_axis_diff_r_data = y_axis_diff_r_data.pop(y_axis_diff_val_pos-1)
        
        # Nombre del eje secundario diferente
        axyrig.set_ylabel(f'{y_axis_label[1]}',fontsize=axis_font_size,labelpad=10)
    ...
    if y_axis_diff == False:
        ...
        y_min = min(y_axis_diff_r_data)
        y_max = max(y_axis_diff_r_data)
