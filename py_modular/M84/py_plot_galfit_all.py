...
Implementar:
    ...
    - Copiar plots a una carpeta externa (crear)
...
def main():
    ...
    csv_folder_path = f'{cwd}/m84_VBIN018_SL_zSimJ_csv'
    ...
    for filter_system_index, filter_system in enumerate(filter_name_group_sorted):
        ...
        '''
        Esto habría que cambiarlo a un diccionario con 
        los nombres de los filtros para que sea más sencillo seleccionar
        los filtros que se quiere plotear
        '''
        df_filter = []    
        ...
        df_filter_dict = {}
        df_filter_dict[filter_name_title] = {}
        ...
        for filter_name in filter_system:
            ...
            df_filter_dict[filter_name_title][filter_name] = pd.read_csv(path_csv, sep=',')
            
            df_index = df_index + 1
            
        '''
        Aquí entonces podríamos cortar el bucle para tener ya el diccionario
        con los dataframes de cada filtro en cada sistema de filtros y a partir
        de este ya hacer los plots con los sistemas que se seleccionen
        '''
        ...
        for l in range(len(plots_list)):
            ...
            for k in range(len(plots_list[l])):
                ...
                plt.savefig(f'{plot_path}/{galaxy}_plot_{version}_{psf_size}_{filter_name_title}_{plots_list[l][k]}.pdf', 
...
if __name__ == '__main__':
    ...
    fits_path = f'{cwd}/{galaxy}_original_fits'
    
    plot_path = f'{cwd}/m84_VBIN018_SL_zSimJ_plots'
    create_folder(plot_path,overwrite=True)
